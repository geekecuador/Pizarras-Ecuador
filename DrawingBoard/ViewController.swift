...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //UIBarButtonItem(title: "Aceptar", style:.Plain, target: self, action: "endSetting")
        self.toolbarEditingItems = [
            UIBarButtonItem(barButtonSystemItem:.FlexibleSpace, target: nil, action: nil),
            UIBarButtonItem(title: "Aceptar", style:.Plain, target: self, action: #selector(ViewController.endSetting))
        ]
    ...
    func image(image: UIImage, didFinishSavingWithError error: NSError?, contextInfo:UnsafePointer<Void>) {
        if let err = error {
            UIAlertView(title: "Error", message: err.localizedDescription, delegate: nil, cancelButtonTitle: "Aceptar").show()
        } else {
            UIAlertView(title: "Exitoso", message: "Se ha guardado correctamente", delegate: nil, cancelButtonTitle: "Aceptar").show()
        }
    }
